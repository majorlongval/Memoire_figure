...
alpha_0 = pi/4;
pc_0 = pi/6;
% Calcul des valeurs qui maximisent
x0 = [alpha_0,pc_0];
[pc_sol, alpha_sol] = opti_max_surf_phic_alpha(rc, R, r, ub, lb, x0);
...
function [pc_sol, alpha_sol] = opti_max_surf_phic_alpha(rcspan, R, r,  ub, lb, x0)
        fun = @(x)-mean_calc_area_SW_new(x(1),R,r,rcspan,x(2));
        A = [];
        b = [];
        Aeq = [];
        beq = [];
...
function mean_area = mean_calc_area_SW_new(alpha,R,r,rcspan,phic)
        area = zeros(1,length(rcspan));
    for i =1:length(rcspan)
        area(i) = calc_area_SW_new(alpha,R,r,rcspan(i),phic);
    end
    mean_area = mean(area);
end